...
async function SuspendedComponent({ searchParams, params }: Props) {
  ...
  const search = success ? data : { jobIds: [] }
          ...
          <div className="flex flex-col gap-1">
            ...
            {jobListing.postedAt != null && (
              <div className="text-sm font-medium text-primary @min-md:hidden">
                <Suspense fallback={`${jobListing.postedAt.getMonth() + 1}/${jobListing.postedAt.getDate()}/${jobListing.postedAt.getFullYear()}`}>
...
const data = await db
  ...
  const transformedData = data.map(row => ({
    ...
    organization: {
      ...
      imageUrl: row.organizationImageUrl || null,
    },
  }))
